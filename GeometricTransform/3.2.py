...
def save_results(img_translation, img_similarity, img_affine, img_b):
    """結果を保存する"""
    cv2.imwrite(
        "images/3.2/translation.jpg",
        cv2.addWeighted(img_translation, 0.5, img_b, 0.5, 0),
    )
    cv2.imwrite(
        "images/3.2/similarity.jpg", cv2.addWeighted(img_similarity, 0.5, img_b, 0.5, 0)
    )
    cv2.imwrite(
        "images/3.2/affine.jpg", cv2.addWeighted(img_affine, 0.5, img_b, 0.5, 0)
    )


def main():
    # 画像の読み込みとグレースケール化
    img_a = cv2.imread("images/a.jpg", cv2.IMREAD_GRAYSCALE)
    img_b = cv2.imread("images/b.jpg", cv2.IMREAD_GRAYSCALE)

    # グレースケール画像の保存
    cv2.imwrite("images/3.2/a_gray.jpg", img_a)
    cv2.imwrite("images/3.2/b_gray.jpg", img_b)
    ...
    # マッチング結果の可視化
    visualize_matches(img_a, kp_a, img_b, kp_b, good_matches, "images/3.2/matches.jpg")
